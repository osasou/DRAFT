...
%parameters see the Encoder and Decoder classes
    ...
    for trial = 1:trials
        [propfound_trial, h_dist, h_all_out] = chirrup_test(re,m,p,K,EbN0, input_bits, Y, h_all_in);
...
function [propfound_trial, h_dist_trial, h_all_out] = chirrup_test(re,m,p,K,EbN0,input_bits, Y, h_all_in)
    %runs a single test
    %;‚ğ‚Â‚¯‚é‚Æˆ—‚ğ‰B‚¹‚é
    h_all_in = [];
    h_all_out = [];
