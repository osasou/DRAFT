...
%          [cluster1,cluster2,cluster3] = hard_clustering(data2.',user_num);
         [clusters] = k_means_plus(data2.',user_num);
                  % 連結する indを出す
%          ind = [];
%          for i=1:user_num
%             ind(i,1) = find(clusters(:,i) == 1);
%          end
%          % 1の数を数えて，% indの数が同じか調べる．
%          % 数える
%          
%          max_size = 0;
%          min_size = 1;
...
%              size = [size(ind1,1),size(ind2,1),size(ind3,1),size(ind4,1),size(ind5,1)];
%              [max_size, max_ind] = max(size);
%              [min_size, min_ind] = min(size);
...
for i = 1:(EbN0-start_ebn0)/2+1
    disp(["EbN0",(i-1)*2+start_ebn0,"dB","prop_packet",1-prop_packet_list(i), "h_dist",dist_list(i)]);
end
