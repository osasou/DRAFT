...
trials = 50000;
slots = 3;
user_num = 5 %ここ変えるならEncoderのhも変える
prop = 1;
m = 8
p = 0;
skip_to_last_EbN0 = 0;
...
%         pro = max(compare_bits(mesg, rec));
        ...
        pro1_bit = min([ber_compare_bits(mesg1, rec1) ber_compare_bits(mesg1, rec2) ber_compare_bits(mesg1, rec3) ber_compare_bits(mesg1, rec4) ber_compare_bits(mesg1, rec5) ]);
        pro2_bit = min([ber_compare_bits(mesg2, rec1) ber_compare_bits(mesg2, rec2) ber_compare_bits(mesg2, rec3) ber_compare_bits(mesg2, rec4) ber_compare_bits(mesg2, rec5) ]);
        pro3_bit = min([ber_compare_bits(mesg3, rec1) ber_compare_bits(mesg3, rec2) ber_compare_bits(mesg3, rec3) ber_compare_bits(mesg3, rec4) ber_compare_bits(mesg3, rec5) ]);
        pro4_bit = min([ber_compare_bits(mesg4, rec1) ber_compare_bits(mesg4, rec2) ber_compare_bits(mesg4, rec3) ber_compare_bits(mesg4, rec4) ber_compare_bits(mesg4, rec5)]);
        pro5_bit = min([ber_compare_bits(mesg5, rec1) ber_compare_bits(mesg5, rec2) ber_compare_bits(mesg5, rec3) ber_compare_bits(mesg5, rec4) ber_compare_bits(mesg5, rec5)]);
        
%         if (pro1_bit+pro2_bit+pro3_bit ~= 0)
%             disp("not zero")
%         end
          this_wrong_bit = (pro1_bit+pro2_bit+pro3_bit+pro4_bit+pro5_bit)/user_num;
          this_prop = (pro1+pro2+pro3+pro4+pro5)/user_num;
