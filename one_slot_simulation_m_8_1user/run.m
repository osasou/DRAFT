...
function [propfound, input_bits_all, output_bits_all, ave_dist, h_output] = run(re, m, p, EbN0, K, slot_trials, without_h, without_sigma)
    addpath('utils');
...
%parameters see the Encoder and Decoder classes
    ...
    for trial = 1:slot_trials
        [propfound_trial, input_bits, output_bits, h_dist, h_all_out] = chirrup_test(re,m,p,K,EbN0, without_h, without_sigma);
...
function [propfound_trial, input_bits, output_bits, h_dist_trial, h_all_out] = chirrup_test(re,m,p,K,EbN0, without_h, without_sigma)
    %runs a single test
    %;‚ğ‚Â‚¯‚é‚Æˆ—‚ğ‰B‚¹‚é
    h_all_in = [];
    h_all_out = [];
