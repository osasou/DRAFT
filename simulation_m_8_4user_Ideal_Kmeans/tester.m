...
trials = 100000;
slots = 3;
user_num = 4 %ここ変えるならEncoderのhも変える
prop = 1;
m = 8
p = 0;
skip_to_last_EbN0 = 0;
...
%         132は44bit×3slots
        ...
        rec3 = zeros(132,one_output_num);
        rec4 = zeros(132,one_output_num);
...
%         for rec3
        ...
        for j=1:one_output_num
%             disp(cast(floor((j-1)/user_num),'int16')+1)
            rec3(45:88,j) = output_bits_all(:,cast(floor((j-1)/user_num),'int16')+1,2);
        end
        for k=1:one_output_num
%             disp(rem(k-1,user_num)+1);
            rec3(89:132,k) = output_bits_all(:,rem(k-1,user_num)+1,3);
        end
        
%         for rec4
        for i=1:one_output_num
            rec4(1:44,i) = output_bits_all(:,4,1);
        end
        for j=1:one_output_num
%             disp(cast(floor((j-1)/user_num),'int16')+1)
            rec4(45:88,j) = output_bits_all(:,cast(floor((j-1)/user_num),'int16')+1,2);
        end
        for k=1:one_output_num
%             disp(rem(k-1,user_num)+1);
            rec4(89:132,k) = output_bits_all(:,rem(k-1,user_num)+1,3);
        end
...
%         pro = max(compare_bits(mesg, rec));
        pro1 = max([compare_bits(mesg1, rec1) compare_bits(mesg1, rec2) compare_bits(mesg1, rec3) compare_bits(mesg1, rec4)]);
        pro2 = max([compare_bits(mesg2, rec1) compare_bits(mesg2, rec2) compare_bits(mesg2, rec3) compare_bits(mesg2, rec4)]);
        pro3 = max([compare_bits(mesg3, rec1) compare_bits(mesg3, rec2) compare_bits(mesg3, rec3) compare_bits(mesg3, rec4)]);
        pro4 = max([compare_bits(mesg4, rec1) compare_bits(mesg4, rec2) compare_bits(mesg4, rec3) compare_bits(mesg4, rec4)]);
        
        pro1_bit = min([ber_compare_bits(mesg1, rec1) ber_compare_bits(mesg1, rec2) ber_compare_bits(mesg1, rec3) ber_compare_bits(mesg1, rec4)]);
        pro2_bit = min([ber_compare_bits(mesg2, rec1) ber_compare_bits(mesg2, rec2) ber_compare_bits(mesg2, rec3) ber_compare_bits(mesg2, rec4)]);
        pro3_bit = min([ber_compare_bits(mesg3, rec1) ber_compare_bits(mesg3, rec2) ber_compare_bits(mesg3, rec3) ber_compare_bits(mesg3, rec4)]);
        pro4_bit = min([ber_compare_bits(mesg4, rec1) ber_compare_bits(mesg4, rec2) ber_compare_bits(mesg4, rec3) ber_compare_bits(mesg4, rec4)]);
%         if (pro1_bit+pro2_bit+pro3_bit ~= 0)
%             disp("not zero")
%         end
         this_wrong_bit = (pro1_bit+pro2_bit+pro3_bit+pro4_bit)/user_num;
         this_prop = (pro1+pro2+pro3+pro4)/user_num;
