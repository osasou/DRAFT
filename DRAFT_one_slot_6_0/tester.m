...
%             disp(cast(floor((j-1)/3),'int16')+1)
            rec1(45:88,j) = output_bits_all(:,cast(floor((j-1)/3),'int16')+1,2);
        end
        for k=1:9
%             disp(rem(k-1,3)+1);
            rec1(89:132,k) = output_bits_all(:,rem(k-1,3)+1,3);
        end
...
%         for rec2
        ...
        for j=1:9
%             disp(cast(floor((j-1)/3),'int16')+1)
            rec2(45:88,j) = output_bits_all(:,cast(floor((j-1)/3),'int16')+1,2);
        end
        for k=1:9
%             disp(rem(k-1,3)+1);
            rec2(89:132,k) = output_bits_all(:,rem(k-1,3)+1,3);
        end
...
for i = 1:(EbN0-start_ebn0)/2+1
    disp(["EbN0",(i-1)*2+start_ebn0,"dB","BER",ber_list(i),"prop_packet",1-prop_packet_list(i), "h_dist",dist_list(i)]);
end
